...
- **External Bridge 基础** — `agent/external/` 提供通用外部平台适配骨架、入站去重、inbox/outbox 状态和 runtime 事件；当前不内置任何具体平台实现。外部消息汇总到默认会话。
...
- 新增外部平台适配：优先实现 `agent/external/ExternalAdapter` 并接入 `ExternalBridgeService`；汇入默认会话，每个会话拥有独立对话历史。

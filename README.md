...
- **流式 WebUI** — 网页聊天通过 WebSocket 接收 `message_delta`、`tool_call`、`tool_result`、`subagent_*` 等事件；事件持久化到 `memory/runtime/events.jsonl`，刷新或后端重启后按 seq 回放未压缩会话细节；Chat 停止按钮与 `/stop` 会通过统一 active task registry 取消当前 turn / Scheduler run。
...
当前运行中的 Chat turn、Scheduler run 会登记到进程内 active task registry；`POST /api/runtime/stop`、WebUI 停止按钮与 `/stop` 共用这一层取消任务，并发出 `runtime_task_cancelled` 事件。刷新后未完成轨迹仍由 runtime event log 还原，取消后的工具 / 子代理段会显示为已中断。
...
| `/stop` | 停止当前运行中的 turn / Scheduler 任务 |

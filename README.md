...
`desktop/` 是一个独立的 Electron 桌面壳，把整套 WebUI 以桌面应用形态交付；**浏览器访问 `:8765` 的网页端不受任何影响**。它会在启动时探测后端是否已运行：未运行则自动 `spawn` 一个 `emperor-agent web` 子进程（退出时回收），已运行则直接附着（不回收）。窗口通过 `loadURL('http://127.0.0.1:8765')` 同源加载完整前端，因此前端源码零改动，相对 `/api` 与 WebSocket 原样可用。

```bash
# 前置：仓库根已 pip install -e . 且 webui/dist/ 已构建
cd desktop
npm install        # 安装 Electron（首次）
npm start          # 启动桌面 App（自动拉起后端）
npm test           # 运行单元测试（node --test，无需额外依赖）
```

需要 Node ≥ 18。可用环境变量 `EMPEROR_BACKEND_CMD` 覆盖后端启动命令（如 `python -m agent.webui`）；host/port 默认从仓库根 `emperor.local.json` 的 `webui.host` / `webui.port` 读取，也可用 `--port` 或 `EMPEROR_WEBUI_PORT` 覆盖。当前仅提供可运行 dev 壳，未包含安装包构建。
...
desktop/                        Electron 桌面壳：自动拉起后端 + 同源加载完整 WebUI（需单独 npm install）
├── package.json               main=main.js, test=node --test, electron 依赖
├── config.js                  解析 root/host/port → backendBaseUrl
├── backend-command.js         构造后端 spawn 命令（venv/env/PATH 解析）
├── health.js                  probeBackend / waitForBackend 就绪轮询
├── lifecycle.js               attach vs spawn 决策 + 退出回收决策
├── window-bounds.js           窗口尺寸归一化与读写（memory/desktop/window.json）
├── main.js                    Electron 装配层（串联上述模块）
├── preload.js                 最小 contextBridge（version/platform）
└── test/                      node:test 单元测试（config/backend-command/health/lifecycle/window-bounds）
```
...
- **不要提交**：`memory/`、`.team/`、`templates/USER.local.md`、`model_config.json`、`mcp_config.json`、`emperor.local.json`、`.env`、`webui/dist/`、`webui/node_modules/`、`desktop-pet/node_modules/`、`desktop/node_modules/`、任何 `*.local.md`。

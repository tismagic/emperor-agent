...
const HIGH_RISK_FALLBACK = /(\bgit\s+push\b|\bgh\s+(pr\s+merge|release|workflow|run)\b|\brm\s+(-[^\s]*r|-[^\s]*f|--recursive|--force)\b|\bsudo\b|\bchmod\b|\bchown\b|\bdeploy\b|\bpublish\b|\brelease\b|\bnpm\s+(install|publish)\b|\bpip\s+install\b|\bbrew\s+install\b|\bdocker\s+(push|compose\s+up|run)\b|\bkubectl\b|\bterraform\s+(apply|destroy)\b)/i
...
const SENSITIVE_PATH_PARTS = new Set(['.emperor', '.git', '.team', 'memory', 'node_modules'])
...
function safeCommandParts(command: string): string[] | null {
  ...
  if (parsed.unsupported || parsed.hasControlOperator || parsed.hasRedirection) return null
...
function parseShellCommand(command: string): ParsedCommand {
  ...
  if (current.length) segments.push(current)
  return { segments, hasControlOperator, hasRedirection, unsupported: tokens.unsupported }
}

function tokenizeShell(command: string): { tokens: Array<{ kind: 'word' | 'control' | 'redirection'; value: string }>; unsupported: boolean } {
  const input = String(command || '').trim()
  const tokens: Array<{ kind: 'word' | 'control' | 'redirection'; value: string }> = []
  ...
  for (let index = 0; index < input.length; index++) {
    ...
    if (ch === '\'' || ch === '"') {
      ...
      for (index += 1; index < input.length; index++) {
        ...
        if (qch === quote) { closed = true; break }
    ...
    if (two === '&&' || two === '||' || two === '>>' || two === '<<') {
      flushWord()
      tokens.push({ kind: two === '&&' || two === '||' ? 'control' : 'redirection', value: two })
      index += 1
      continue
    }
    if (ch === ';' || ch === '|') {
...
function isHighRiskSegment(segment: string[]): boolean {
  ...
  if (head === 'gh' && (sub === 'release' || sub === 'workflow' || sub === 'run' || (sub === 'pr' && parts[2] === 'merge'))) return true
  if (head === 'rm' && parts.slice(1).some((part) => part === '--recursive' || part === '--force' || /^-[A-Za-z]*[rf][A-Za-z]*$/.test(part))) return true
  ...
  if (head === 'docker' && (sub === 'push' || sub === 'run' || (sub === 'compose' && parts[2] === 'up'))) return true
  if (head === 'terraform' && (sub === 'apply' || sub === 'destroy')) return true
  return false
}

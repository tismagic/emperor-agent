 ...
 */
export interface OpenAiMsg { role: string; content?: unknown; tool_calls?: unknown; tool_call_id?: string; name?: string; [k: string]: unknown }

export function pairToolCalls(history: OpenAiMsg[]): [OpenAiMsg[], number, number] {
  ...
  function flushExpected(): void {
    for (const [id, name] of expected) {
      cleaned.push({ role: 'tool', tool_call_id: id, name, content: '(tool execution interrupted)' })
      filled++
    }
    expected.length = 0
  }

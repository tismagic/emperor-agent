...
import { buildUserContent, encodeForOpenAIBlock, refToJson, type UserContent } from './encode'
...
describe('AttachmentStore (agent/attachments.py parity)', () => {
  it('saves text attachments with safe names, sidecars, ids, and lookup by id', () => {
    ...
    expect(loaded.name).toBe(basename(ref.rel_path).replace(/^[0-9a-f]{8}-/, ''))
  })

  it('validates mime and size limits', () => {
    const store = new AttachmentStore(tmp('emperor-attachments-limits-'))
    expect(() => store.save({ raw: Buffer.from('x'), name: 'bad.exe', mime: 'application/x-msdownload' })).toThrow(/unsupported mime/)
    expect(() => store.save({ raw: Buffer.alloc(MAX_IMAGE_BYTES + 1), name: 'big.png', mime: 'image/png' })).toThrow(/file too large/)
  })
  ...
  it('extracts pdf text through the injected extractor without blocking uploads when absent', () => {
    ...
    const noText = noExtractor.save({ raw: pdf, name: 'doc.pdf', mime: 'application/pdf' })
    expect(noText.kind).toBe('document')
    expect(noText.has_text).toBe(false)

    const withExtractor = new AttachmentStore(root, { pdfTextExtractor: () => 'PDF text' })
    const withText = withExtractor.save({ raw: Buffer.from('%PDF-1.4\nother', 'utf8'), name: 'doc.pdf', mime: 'application/pdf' })
  ...
  it('truncates sidecar text like the Python store', () => {
    ...
    const ref = store.save({ raw: Buffer.from(text), name: 'long.txt', mime: 'text/plain' })
...
describe('attachment encoding and chat content assembly', () => {
  it('encodes image refs to OpenAI image_url blocks and JSON refs', () => {
    ...
    const ref = store.save({ raw: Buffer.from([1, 2, 3, 4]), name: 'pic.png', mime: 'image/png' })
  ...
  it('builds model user content for document text plus vision image blocks', () => {
    ...
    const text = store.save({ raw: Buffer.from('line one'), name: 'notes.txt', mime: 'text/plain' })
    const image = store.save({ raw: Buffer.from([255]), name: 'x.jpg', mime: 'image/jpeg' })

    const content = buildUserContent('see attached', [text.id, image.id], store, { supportsVision: true })
    expect(Array.isArray(content)).toBe(true)
    const blocks = content as Exclude<UserContent, string>
    expect(blocks[0]).toMatchObject({ type: 'text' })
    expect(String((blocks[0] as { text: string }).text)).toContain('[附件 notes.txt 提取文本]')
    expect(String((blocks[0] as { text: string }).text)).toContain(`[已落盘: ${text.rel_path}]`)
  ...
  it('keeps non-vision image attachments visible as text fallback', () => {
    ...
    const image = store.save({ raw: Buffer.from([9]), name: 'x.webp', mime: 'image/webp' })
    const content = buildUserContent('', [image.id], store, { supportsVision: false })

...
import { buildStdioEnv, MCPConnection, type MCPCallToolResult, type MCPToolDefinition } from './connection'
...
class FakeConnection extends MCPConnection {
  ...
  constructor(serverName: string, tools: MCPToolDefinition[], opts: { connectOk?: boolean; output?: MCPCallToolResult | string } = {}) {
    super(serverName)
    this.tools = tools
    this.connectOk = opts.connectOk ?? true
    this.output = typeof opts.output === 'string'
  ...
  override async callTool(toolName: string, args: Record<string, unknown>): Promise<MCPCallToolResult> {
...
describe('MCP config', () => {
  it('loads defaults, deep-merges mcp_config.json, and expands env placeholders', () => {
    ...
    writeFileSync(join(root, 'mcp_config.json'), JSON.stringify({
    ...
    expect(cfg.defaults).toMatchObject({ read_only: true, exclusive: false, max_result_chars: 777 })
    ...
    expect(cfg.servers.alpha!.tool_overrides.search).toEqual({ read_only: false })
  })
  ...
  it('validates and writes raw config compatibly', () => {
    ...
    expect(JSON.parse(readFileSync(join(root, 'mcp_config.json'), 'utf8')).defaults.read_only).toBe(true)
  })
})
...
describe('MCP connection env', () => {
  it('keeps only safe inherited env and explicit server env', () => {
    const env = buildStdioEnv(
      ...
      { PATH: '/bin', HOME: '/Users/me', OPENAI_API_KEY: 'leak', LANG: 'en_US.UTF-8' },
    )
    expect(env).toEqual({
      PATH: '/custom/bin',
      HOME: '/Users/me',
...
describe('MCP adapter/client', () => {
  it('wraps tools as emperor Tool instances', async () => {
    ...
    expect(await adapter.execute({ q: 'hello' })).toMatchObject({
      isError: false,
      metadata: { mcp: true, untrusted: true, server: 'alpha', tool: 'mcp_alpha_search', mcp_tool: 'search' },
    })
    expect(conn.called).toEqual([{ tool: 'search', args: { q: 'hello' } }])
  })

  it('marks MCP protocol errors as failed untrusted tool results', async () => {
    const conn = new FakeConnection('alpha', [], { output: { content: 'remote failed', isError: true } })
    await conn.connect()
    const adapter = new MCPToolAdapter({
      serverName: 'alpha',
      toolName: 'search',
    ...
    expect(result.metadata).toMatchObject({ mcp: true, untrusted: true, server: 'alpha', tool: 'mcp_alpha_search', mcp_tool: 'search' })
  ...
  it('initializes enabled servers, ignores failures, applies overrides, and registers tools', async () => {
    ...
    writeFileSync(join(root, 'mcp_config.json'), JSON.stringify({
      ...
      servers: {
        alpha: { enabled: true, transport: 'stdio', tool_overrides: { search: { read_only: false, exclusive: true, max_result_chars: 456 } } },
        beta: { enabled: true, transport: 'sse' },
        off: { enabled: false, transport: 'stdio' },
      },
    }), 'utf8')
    const conns: Record<string, FakeConnection> = {
      alpha: new FakeConnection('alpha', [{ name: 'search', description: 'Search', inputSchema: { type: 'object', properties: {}, required: [] } }]),
      beta: new FakeConnection('beta', [{ name: 'lookup', description: 'Lookup', inputSchema: { type: 'object', properties: {}, required: [] } }], { connectOk: false }),
      off: new FakeConnection('off', [{ name: 'disabled', description: '', inputSchema: { type: 'object', properties: {}, required: [] } }]),
    }
    const client = new MCPClient(root, { connectionFactory: (cfg) => conns[cfg.name]! })
    ...
    expect(await registry.execute('mcp_alpha_search', { q: 'x' })).toContain('ok:search')

...
export function loadMcpConfig(root: string, env: Record<string, string | undefined> = process.env): MCPConfig {
  const path = join(root, MCP_CONFIG_FILE)
  const raw = structuredClone(DEFAULT_MCP_CONFIG) as Record<string, unknown>
  if (existsSync(path)) {
    const loaded = JSON.parse(readFileSync(path, 'utf8') || '{}') as Record<string, unknown>
    deepMerge(raw, expandEnv(loaded, env) as Record<string, unknown>)
  }
  return parseConfig(raw)
}

export function saveMcpConfig(root: string, raw: Record<string, unknown>): void {
  if (!raw.servers || typeof raw.servers !== 'object' || Array.isArray(raw.servers)) throw new Error("mcp_config: 'servers' must be an object")
  const data = { ...raw }
  if (!data.defaults || typeof data.defaults !== 'object' || Array.isArray(data.defaults)) data.defaults = DEFAULT_MCP_CONFIG.defaults
  writeFileSync(join(root, MCP_CONFIG_FILE), JSON.stringify(data, null, 2) + '\n', 'utf8')
}

export function expandEnv(value: unknown, env: Record<string, string | undefined> = process.env): unknown {
...
function parseConfig(raw: Record<string, unknown>): MCPConfig {
  const serversRaw = raw.servers && typeof raw.servers === 'object' && !Array.isArray(raw.servers)

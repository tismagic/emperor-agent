...
function decision(value: 'deny' | 'allow' | 'passthrough', reason = ''): HookAggregateDecision {
...
describe('TaskManager hook transitions', () => {
  it('does not persist a TaskCreated candidate when hooks deny it', async () => {
    ...
    const manager = new TaskManager(root(), { hooks: host(async (event) => {
      events.push(event)
      return event === 'TaskCreated' ? decision('deny', 'no task') : decision('passthrough')
    }) })

    const created = await manager.startTaskWithHooks({
      kind: TaskKind.SUBAGENT, title: 'candidate', source: 'test', sessionId: 's1',
    })
  ...
  it('persists an allowed candidate and rejects completion without mutating running state', async () => {
    const manager = new TaskManager(root(), { hooks: host(async (event) => event === 'TaskCompleted'
      ? decision('deny', 'not complete')
      : decision('allow')) })
    const created = await manager.startTaskWithHooks({
      kind: TaskKind.SUBAGENT, title: 'allowed', source: 'test', sessionId: 's1',
    })
    expect(created?.status).toBe(TaskStatus.RUNNING)

    const denied = await manager.completeTaskWithHooks(created!.id, { summary: 'done' })
  ...
  it('commits TaskCompleted only after hook approval', async () => {
    const manager = new TaskManager(root(), { hooks: host(async () => decision('allow')) })
    const created = await manager.startTaskWithHooks({
      kind: TaskKind.SUBAGENT, title: 'allowed', source: 'test', sessionId: 's1',
    })

    const completed = await manager.completeTaskWithHooks(created!.id, { summary: 'done' })

    expect(completed).toMatchObject({ committed: true, record: { status: TaskStatus.COMPLETED } })

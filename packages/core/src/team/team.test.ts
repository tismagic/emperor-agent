...
import { LEAD_ACTOR, TeamMember, TeamMessage, TeamStatus, validateActorName, validateMemberName } from './models'
import { TeamStore } from './store'
import { TeamBroadcastTool, TeamListTool, TeamReadInboxTool, TeamSendMessageTool, TeamShutdownTool, TeamSpawnTool } from './tools'
...
describe('team models/events', () => {
  it('validates names, normalizes members/messages, and creates event payloads', () => {
    ...
    const member = TeamMember.fromDict({ name: 'alice', role: 'reader', agentType: 'sili_suitang', status: 'bogus' })
    expect(member.status).toBe(TeamStatus.IDLE)
    const msg = TeamMessage.create({ from_actor: 'lead', to: 'alice', content: 'hi', type: 'task' })
    expect(TeamMessage.fromDict(msg.toDict()).toDict()).toEqual(msg.toDict())
    expect(teamEvents.memberUpdate(member)).toMatchObject({ event: 'team_member_update', member: { name: 'alice' } })
    expect(teamEvents.messageEvent(msg)).toMatchObject({ event: 'team_message', message: { to: 'alice' } })
    expect(teamEvents.runStart({ parent_id: 'p', member, purpose: 'work' })).toMatchObject({ event: 'team_run_start', teammate: 'alice' })
  })
})
...
describe('TeamStore and MessageBus', () => {
  it('persists roster, threads, checkpoints, cursors, and marks stale working offline', () => {
    ...
    const member = new TeamMember({ name: 'alice', role: 'reader', agent_type: 'sili_suitang', status: TeamStatus.WORKING })
    ...
    reopened.writeCheckpoint('alice', [{ role: 'user', content: 'pending' }], { pending_cursor_start: 1, pending_cursor_end: 2, pending_message_ids: ['m1'] })
    reopened.writeCursor('alice', 3)
    expect(reopened.readThread('alice')).toHaveLength(1)
    expect(reopened.readCheckpointPayload('alice')?.pending_message_ids).toEqual(['m1'])
  ...
  it('isolates a corrupt config.json instead of silently discarding it (audit P1-5)', () => {
    ...
    const store = new TeamStore(root)
    store.upsertMember(new TeamMember({ name: 'alice', role: 'reader', agent_type: 'sili_suitang' }))
  ...
  it('appends inbox messages, reads unread by cursor, and skips corrupt lines', () => {
    ...
    expect(bus.read('alice', { limit: 1 }).map((m) => m.content)).toEqual(['one'])
    expect(bus.unreadCount('alice')).toBe(1)
    expect(bus.read('alice', { limit: 0, mark_read: false }).map((m) => m.content)).toEqual(['two'])
    expect(bus.unreadCount('alice')).toBe(1)
  })
  ...
  it('rotates the read prefix of an inbox once it grows past the hot threshold, without losing unread messages (audit P1-4)', () => {
    ...
    const bus = new MessageBus(store)
    const total = 5200
    for (let i = 0; i < total; i++) bus.send({ from_actor: 'lead', to: 'alice', content: `msg-${i}` })
    ...
    const hotLines = readFileSync(store.inboxPath('alice'), 'utf8').split('\n').filter((l) => l.trim())
    ...
    expect(readdirSync(archiveDir).some((f) => f.startsWith('alice'))).toBe(true)
  })
})
...
describe('TeamManager and tools', () => {
  it('spawns teammates, wakes on messages, writes lead replies, and exposes payloads', async () => {
    ...
    const manager = new TeamManager({
      ...
      runnerFactory: ({ member }) => ({
        step: (history: Array<Record<string, unknown>>) => `handled by ${member.name}: ${String(history.at(-1)?.content ?? '').slice(0, 20)}`,
      }),
      eventSink: async (event) => { emitted.push(event) },
    })

    expect(roleToAgentType('coder')).toBe('neiguan_yingzao')
    const created = JSON.parse(await manager.spawnTeammate({ name: 'alice', role: 'reader', task: 'read docs' }))
    ...
    const sent = JSON.parse(await manager.sendMessage({ to: 'alice', content: 'next task', wake: true }))
    expect(sent.result).toContain('handled by alice')
    expect(manager.store.readThread('alice').at(-1)?.role).toBe('user')
    expect(manager.readInbox({ actor: LEAD_ACTOR })).toContain('handled by alice')
  })
  ...
  it('team tools delegate to the manager with lead/teammate wake boundaries', async () => {
    ...
    expect(await new TeamSendMessageTool(manager).execute({ to: 'bob', content: 'hi', wake: true })).toContain('ok bob')
    expect(await new TeamBroadcastTool(manager).execute({ content: 'all', wake: false })).toContain('"sent"')
    expect(await new TeamReadInboxTool(manager).execute({ limit: 10, mark_read: false })).toContain('ok bob')
    expect(await new TeamShutdownTool(manager).execute({ name: 'bob' })).toContain('"shutdown"')

    const teammateSend = new TeamSendMessageTool(manager, { sender: 'bob', allowWake: false })
    const result = await teammateSend.execute({ to: LEAD_ACTOR, content: 'report', wake: true })

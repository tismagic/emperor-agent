;(function expose(factory) {
  ...
  function interactionDetail(interaction, kind) {
    ...
    if (kind === 'plan')
      return firstText(interaction, ['title', 'summary', 'plan_markdown'])
    const firstQuestion = Array.isArray(interaction.questions)
      ? interaction.questions[0]
      : null
    return (
      firstText(firstQuestion, ['question', 'header']) ||
      firstText(interaction, ['context', 'title', 'summary'])
    )
  }
  ...
  function toolDetail(name, args) {
    ...
    if (tool === 'read_file')
      return fileName(firstText(input, ['path', 'file', 'relative_path']))
    if (tool === 'grep') {
    ...
    if (tool === 'load_skill')
      return firstText(input, ['name', 'skill', 'path'])
    if (tool === 'run_command') return firstText(input, ['command', 'cmd'])
    if (tool === 'scheduler')
      return firstText(input, ['action', 'name', 'job_id'])
    if (tool === 'web_fetch')
      return urlDisplay(firstText(input, ['url', 'href']))
  ...
  function bubbleForTool(name, args) {
    ...
    if (tool === 'read_file')
      return bubbleWithDetail('正在读文件：', detail, '正在查阅资料。')
    if (tool === 'grep')
      return bubbleWithDetail('正在搜索：', detail, '正在查阅资料。')
    if (tool === 'glob')
      return bubbleWithDetail('正在匹配文件：', detail, '正在查阅资料。')
    if (tool === 'write_file' || tool === 'edit_file')
      return bubbleWithDetail('正在编辑：', detail, '正在整理改动。')
    if (tool === 'update_todos')
      return bubbleWithDetail('正在更新：', detail, '正在更新待办。')
    if (tool === 'load_skill')
      return bubbleWithDetail('正在加载技能：', detail, '正在加载技能。')
    if (tool === 'run_command')
      return bubbleWithDetail('正在运行命令：', detail, '正在运行命令。')
    if (tool === 'scheduler')
      return bubbleWithDetail(
        '正在处理定时任务：',
        detail,
        '正在处理定时任务。',
      )
    if (tool === 'web_fetch')
      return bubbleWithDetail('正在查看网页：', detail, '正在看看网页。')
    if (
      tool === 'dispatch_subagent' ||
      tool.includes('team') ||
      tool.includes('broadcast')
    ...
    return bubbleWithDetail(
      '正在处理工具：',
      detail || name,
      TOOL_BUBBLES[toolAnimation(name)] || '正在处理工具。',
    )
  }
  ...
  function mapRuntimeEvent(event) {
    ...
    if (type === 'user_message')
      return { animation: 'thinking', bubble: '收到，开始想。' }
    ...
    if (type === 'tool_call')
      return {
        animation: toolAnimation(event.name),
        bubble: bubbleForTool(event.name, event.arguments),
      }
    if (type === 'tool_result') {
      return {
        bubble: bubbleForContent('工具完成：', event.summary, '工具完成。'),
    ...
    if (type === 'subagent_tool_call') {
      return {
        animation: toolAnimation(event.name),
        bubble: bubbleForContent(
          '子代理用工具：',
          bubbleForTool(event.name, event.arguments),
          '子代理正在用工具。',
        ),
      }
    }
    ...
    if (type === 'subagent_start' || type === 'team_run_start') {
      ...
      const detail = [who, firstText(event, ['purpose'])]
        .filter(Boolean)
        .join('：')
      return {
        animation: 'conducting',
        bubble: bubbleWithDetail('正在派遣队友：', detail, '正在派遣队友。'),
        subagentDelta: 1,
      }
    }
    ...
    if (type === 'team_run_delta') {
      return {
        animation: 'conducting',
        bubble: bubbleForContent('队友回报：', event.delta, '队友正在回报。'),
        bubbleDurationMs: 2200,
      }
    }
    ...
    if (type === 'team_run_tool_result') {
      return {
        bubble: bubbleForContent('队友完成：', event.summary, '队友工具完成。'),
        bubbleDurationMs: 2600,
      }
    }
    if (type === 'subagent_done' || type === 'team_run_done') {
      return {
        animation: 'typing',
        bubble: bubbleForContent('队友回报了：', event.summary, '队友回报了。'),
        subagentDelta: -1,
      }
    }
    ...
    if (type === 'ask_request') {
      return {
        ...
        bubble: bubbleForContent(
          '需要主人拍板：',
          interactionDetail(event.interaction, 'ask'),
          '需要主人拍板。',
        ),
        bubbleDurationMs: 0,
      }
    ...
    if (type === 'plan_draft') {
      return {
        ...
        bubble: bubbleForContent(
          '计划待批：',
          interactionDetail(event.interaction, 'plan'),
          '需要主人拍板。',
        ),
        bubbleDurationMs: 0,
      }
    ...
    if (type === 'turn_paused') {
      return {
        ...
        bubble: bubbleForContent(
          '暂停等待：',
          interactionDetail(event.interaction, event.interaction?.kind),
          '需要主人拍板。',
        ),
        bubbleDurationMs: 0,
      }
    ...
    if (type === 'assistant_done' || type === 'scheduler_run_done') {
      const detail =
        type === 'assistant_done' ? event.content : jobDetail(event.job)
      return {
        animation: 'happy',
        bubble: bubbleForContent('办好了：', detail, '办好了。'),
        resetAfterMs: 4000,
      }
    }
    return null

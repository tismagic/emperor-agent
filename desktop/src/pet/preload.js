...
function readRecentEvents(limit = 100) {
  ...
  return events
    .sort(
      (a, b) =>
        eventTime(a) - eventTime(b) || Number(a.seq || 0) - Number(b.seq || 0),
    )
...
function readControl() {
  const state = safeJson(
    fs.existsSync(path.join(root, 'memory', 'control', 'state.json'))
      ? fs.readFileSync(
          path.join(root, 'memory', 'control', 'state.json'),
          'utf8',
        )
      : '{}',
    {},
...
contextBridge.exposeInMainWorld('emperorPet', {
  ...
  readBootstrap: async () => {
    ...
    for (const file of runtimeFiles())
      cursors.set(file, readEvents(file).length)
    return {
      runtime: {
        latestSeq: Math.max(
          0,
          ...events.map((event) => Number(event.seq || 0) || 0),
        ),
        events,
      },
      control: readControl(),
  ...
  closePet: async () => {
    try {
      ipcRenderer.invoke('emperor:pet:close')
    } catch {
      /* best-effort */
    }
  },

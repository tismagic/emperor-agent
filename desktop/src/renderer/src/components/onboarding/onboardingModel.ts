import type { BootstrapPayload, ModelConfigRaw, ModelEntry, ProviderOption } from '../../types'
...
export function createOnboardingDraft(payload: BootstrapPayload | null | undefined): OnboardingDraft {
  ...
  const provider = String(entry?.provider || current.provider || defaultProvider(modelConfig?.providerOptions)).trim() || 'deepseek'
  const option = providerOption(payload, provider)
  const mainModelId = String(entry?.mainModelId || entry?.id || current.mainModelId || current.model || 'deepseek-chat').trim()
  const secondaryModelId = String(entry?.secondaryModelId || current.secondaryModelId || mainModelId || 'deepseek-chat').trim()
  ...
  return {
    ...
    apiKey: '',
    apiBase: String(entry?.apiBase || current.apiBase || option?.defaultApiBase || '').trim(),
    mainModelId: mainModelId || 'deepseek-chat',
    secondaryModelId: secondaryModelId || mainModelId || 'deepseek-chat',
    maxTokens: numberOr(entry?.maxTokens, current.maxTokens, defaults.maxTokens, 8192),
    temperature: numberOr(entry?.temperature, current.temperature, defaults.temperature, 0.1),
    contextWindowTokens: numberOr(entry?.contextWindowTokens, current.contextWindowTokens, defaults.contextWindowTokens, 128000),
    reasoningEffort: String(entry?.reasoningEffort || current.reasoningEffort || defaults.reasoningEffort || ''),
  }
}
...
export function onboardingValidationErrors(
  ...
  if (!Number.isFinite(draft.maxTokens) || draft.maxTokens <= 0) errors.push('Max Tokens 必须大于 0')
  if (!Number.isFinite(draft.temperature) || draft.temperature < 0) errors.push('Temperature 必须是非负数')
  if (!Number.isFinite(draft.contextWindowTokens) || draft.contextWindowTokens <= 0) errors.push('Context Window 必须大于 0')
  if (
...
export function hasExistingCredential(payload: BootstrapPayload | null | undefined, provider: string): boolean {
  const config = payload?.modelConfig?.config
  if (!config) return false
  const entry = activeRawEntry(config)
  if (entry && String(entry.provider || '') === provider && String(entry.apiKey || '').trim()) return true
  const providerConfig = config.providers?.[provider]
  return Boolean(String(providerConfig?.apiKey || '').trim())
}

function activeRawEntry(config: ModelConfigRaw | null | undefined): ModelEntry | null {
...
function providerOption(payload: BootstrapPayload | null | undefined, provider: string): ProviderOption | undefined {
  return (payload?.modelConfig?.providerOptions || []).find((option) => option.name === provider)
}

function defaultProvider(options: ProviderOption[] | undefined): string {
  return options?.find((option) => option.name === 'deepseek')?.name || options?.[0]?.name || 'deepseek'
}

...
export function applySchedulerEventToBootstrap(
  ...
  boot.scheduler.jobs = jobs
    ...
    .sort(
      (a, b) =>
        Number(a.state?.nextRunAtMs || Infinity) -
        Number(b.state?.nextRunAtMs || Infinity),
    )
  boot.scheduler.status.jobs = boot.scheduler.jobs.length
  boot.scheduler.status.enabled = boot.scheduler.jobs.filter(
    (job) => job.enabled,
  ).length
  boot.scheduler.status.nextRunAtMs =
    boot.scheduler.jobs
      .filter((job) => job.enabled && job.state?.nextRunAtMs)
      .map((job) => Number(job.state.nextRunAtMs))
      .sort((a, b) => a - b)[0] || null
  boot.scheduler.status.lastError =
    boot.scheduler.jobs.find((job) => job.state?.lastStatus === 'error')?.state
      .lastError || null
}

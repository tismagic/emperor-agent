...
- MCP 工具已支持从 `mcp_config.json` 的 `defaults.max_result_chars` 和 `tool_overrides.<tool>.max_result_chars` 读取预算，并通过 `MCPToolAdapter.max_result_chars` 进入同一条 `ToolRegistry.tool_result_limits()` 链路。

...
| 项目执行 / Plan Runtime | Plan Mode 切权限，只读探索，计划文件持久化，ExitPlanMode 审批，TodoWrite 推进，验证失败继续 | 已有 `PlanStore`、`PlanStep`、`PlanDraftState`、`PlanQualityGate`、`plan_runtime_update`、验证 evidence、Final Answer Gate、`PlanDecisionPolicy` 写工具前置 guard | 继续补 Plan Runtime 恢复附件、Evidence Gate 和独立验证子代理 |
